...
server <- function(input, output, session) {
  ...
  output$countriesMap <- renderLeaflet({
    ...
    modifiers <- isolate(all_of(input$textModifier))
    colors <- isolate(input$mod1)
    patterns <- isolate(input$mod2)
    process <- isolate(input$mod3)
    fibers <- isolate(input$mod4)
    geography <- isolate(input$mod5)
    qualities <- isolate(input$mod6)
    inferredQualities <- isolate(input$mod7)
